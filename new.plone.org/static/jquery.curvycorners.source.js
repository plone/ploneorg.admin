...
function curvyObject() {
	...
	this.boxHeight 			= strip_px($(this.box).css("height")) ? strip_px($(this.box).css("height")) : this.box.scrollHeight; 
    this.boxWidth 			= strip_px($(this.box).css("width")) ? strip_px($(this.box).css("width")) : this.box.scrollWidth; 
    this.borderWidth     	= strip_px($(this.box).css("borderTopWidth")) ? strip_px($(this.box).css("borderTopWidth")) : 0; 
	this.boxPaddingTop 		= strip_px($(this.box).css("paddingTop"));
	this.boxPaddingBottom 	= strip_px($(this.box).css("paddingBottom"));
	this.boxPaddingLeft 	= strip_px($(this.box).css("paddingLeft"));
	this.boxPaddingRight 	= strip_px($(this.box).css("paddingRight"));
	...
	// Style/fix
	$(this.box).css({"padding":"0px", "borderColor":this.borderColour, 'overflow':'visible'});
	
    if(boxPosition != "absolute") $(this.box).css("position","relative");
	// Fix IE6 widths
    if(($.browser.msie && $.browser.version==6) && this.boxWidth == "auto" && this.boxHeight == "auto") $(this.box).css("width","100%");
	if(($.browser.msie)) {
			$(this.box).css("zoom","1");
    ...
    this.applyCorners = function() {
		...
		if(this.settings.autoPad == true) {
			...
			$(contentContainer2).css({ margin:"0","padding-bottom":this.boxPaddingBottom,"padding-top":this.boxPaddingTop,"padding-left":this.boxPaddingLeft,"padding-right":this.boxPaddingRight}).addClass('hasBackgroundColor');
			// Float added to remove gaps with negative margins!
            $(contentContainer).css({position:"relative", 'float':"left", width:"100%", "margin-top":"-" + (topMaxRadius-this.borderWidth) + "px", "margin-bottom":"-" + (botMaxRadius-this.borderWidth) + "px"}).html(this.boxContent).addClass = "autoPadDiv";
			$(clearDiv).css("clear","both");
            this.contentdiv = this.box.appendChild(contentContainer2);
                ...
                if(!this.settings[cc]) {
                    ...
                    if(this.masterCorners[this.settings[cc].radius]) {
                        ...
                        for(var intx = 0, j = this.settings[cc].radius; intx < j; intx++) {
                            ...
                            if(y1 > -1) this.drawPixel(intx, 0, this.boxColour, 100, (y1+1), newCorner, -1, this.settings[cc].radius, 0);
                            if(borderRadius != j) {
                                for(var inty = (y1 + 1); inty < y2; inty++) {
                                    if(this.settings.antiAlias) {
                                        if(this.bgImage != "") {
                                            ...
                                            if(borderFract < 30) {
										    	this.drawPixel(intx, inty, this.borderColour, 100, 1, newCorner, 0, this.settings[cc].radius, 1);
                                            } else {
									        	this.drawPixel(intx, inty, this.borderColour, 100, 1, newCorner, -1, this.settings[cc].radius, 1);
                                            };
                                        } else {
                                            var pixelcolour = BlendColour(this.boxColour, this.borderColour, pixelFraction(intx, inty, borderRadius));
                                            this.drawPixel(intx, inty, pixelcolour, 100, 1, newCorner, 0, this.settings[cc].radius, cc, 1);
                                        };
                                    };
                                };
                                if(this.settings.antiAlias) {
                                      ...
                                      {
                                         if (y2 == -1) y2 = 0;
                                         this.drawPixel(intx, y2, this.borderColour, 100, (y3 - y2 + 1), newCorner, 0, 0, 1);
                                      }
                                } else {
                                      if(y3 >= y1)
                                      {
                                          this.drawPixel(intx, (y1 + 1), this.borderColour, 100, (y3 - y1), newCorner, 0, 0, 1);
                                      }
                                };
                            ...
                            if(this.settings.antiAlias) {
                                for(var inty = (y3 + 1); inty < y4; inty++) {
									this.drawPixel(intx, inty, outsideColour, (pixelFraction(intx, inty , j) * 100), 1, newCorner, ((this.borderWidth > 0)? 0 : -1), this.settings[cc].radius, 1);
                                };                                
                            };
                        };                        
                    ...
                    if(this.bottomContainer) {
                        if(this.settings.bl.radius && this.settings.br.radius) {
                            ...
                            if(this.bgImage != "" && topMaxRadius>0)
								...
								$(newFillerBar).css("background-position","-" + (this.settings.bl.radius - this.borderWidth) + "px -" + ($(this.box).height() ) + "px").addClass('hasBackgroundColor');
    ...
    this.drawPixel = function(intx, inty, colour, transAmount, height, newCorner, image, cornerRadius, isBorder) {
